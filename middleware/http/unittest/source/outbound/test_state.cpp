   ...
   namespace http
   {
      namespace outbound
      {
         ...
         TEST( http_outbound_state_request, add_request_headers)
         {
            ...
            {
               state::pending::Request request;
               request.state().header.request.add( casual::header::Fields{{
                  casual::header::Field{ "key-1", "value-1"},
                  casual::header::Field{ "key-2", "value-2"},
                  casual::header::Field{ "key-3", "value-3"},
               }});
