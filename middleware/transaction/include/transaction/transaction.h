...
namespace casual
{
   namespace transaction
   {
      ...
      struct Transaction
      {
         ...
         inline const std::vector< resource::id>& involved() const noexcept { return m_involved;}
         
         //! associate resource
         void involve( resource::id id);
         ...
         inline const std::vector< resource::id>& dynamic() const noexcept { return m_dynamic;}
         
         //! associate id to dynamic registration
         //! @attention will not associate with involved
         //! @return true if id is not registred before
         bool associate_dynamic( resource::id id);
         bool disassociate_dynamic( resource::id id);

         //! associate a pending message reply
         void associate( const common::strong::correlation::id& correlation);

         //! discards a pending reply from this transaction
         void replied( const common::strong::correlation::id& correlation);
         ...
         bool associated( const common::strong::correlation::id& correlation) const noexcept;

         //! associated descriptors to this transaction
         const std::vector< common::strong::correlation::id>& correlations() const noexcept;
         ...
         friend bool operator == ( const Transaction& lhs, const common::strong::correlation::id& rhs);
      ...
      private:
         std::vector< resource::id> m_involved;
         std::vector< common::strong::correlation::id> m_pending;
         std::vector< resource::id> m_dynamic;
