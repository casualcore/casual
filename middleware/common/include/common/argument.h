   ...
   namespace common::argument
   {
      ...
      namespace detail
      {
         ...
         struct Invoke
         {
            ...
            void assign( std::string_view key, range_type values) { m_callable->assign( key, values);}
         ...
         private:
            struct Concept 
            {
               ...
               virtual void assign( std::string_view key, range_type values) = 0;
            ...
            struct model : Concept
            {
               ...
               void assign( std::string_view key, range_type values) override { m_callable.assign( key, values);}
         ...
         struct Handler
         {
            ...
            //! @{
            inline bool has( std::string_view key) const { return m_handler->has( key);}
            inline bool next( std::string_view key) const { return m_handler->next( key);}
            //! @}

            inline void assign( std::string_view key, range_type values) { m_handler->assign( key, values);}
            inline void invoke() { m_handler->invoke();}
            
            inline detail::Invoked completion( std::string_view key, range_type values) { return m_handler->completion( key, values);}
         ...
         private:
            struct Concept 
            {
               ...
               virtual bool next( std::string_view key) const = 0;
               virtual void assign( std::string_view key, range_type values) = 0;
               virtual void invoke() = 0;
               virtual detail::Invoked completion( std::string_view key, range_type values) = 0;
            ...
            struct model : Concept
            {
               ...
               bool has( std::string_view key) const override { return m_handler.has( key);}
               bool next( std::string_view key) const override { return m_handler.next( key);}
               void assign( std::string_view key, range_type values) override { m_handler.assign( key, values);}
               void invoke() override { m_handler.invoke();}
               detail::Invoked completion( std::string_view key, range_type values) override { return m_handler.completion( key, values);}
         ...
         namespace invoke
         {
            namespace implementation 
            {
               ...
               struct value_traits
               {
                  ...
                  { 
                        value = string::from< T>( values.front());
         ...
         struct Holder
         {
            ...
            inline bool next( std::string_view key) const
            {
               return ! algorithm::find_if( m_handlers, [&key]( const auto& h){ return h.next( key);}).empty();
            }

            detail::Invoked completion( std::string_view key, range_type values)
            {
            ...
            inline void assign( std::string_view key, range_type values)
            {
            ...
            {
               return algorithm::transform( m_handlers, []( auto& handler)
               {
         ...
         private:
            ...
            template< typename A>
            auto apply( A&& applier, std::string_view key) -> decltype( applier( std::declval< detail::Handler&>()))
            {
         ...
         struct basic_cardinality
         {
            ...
            template< typename I>
            inline void assign( I&& invocable, std::string_view key, range_type values) 
            { 
            ...
            inline void validate( std::string_view key) const 
            { 
      ...
      struct Option : detail::basic_keys
      {
         ...
         inline bool next( std::string_view key) const { return has( key);}
         ...
         detail::Invoked completion( std::string_view key, range_type values) const
         {
      ...
      struct basic_parse
      {
         ...
         {
            assert( argc > 0);
            basic_parse::operator()( std::vector< std::string_view>( argv + 1, argv + argc));
         }

         void operator() ( std::vector< std::string_view> arguments)
         {
            basic_parse::operator()( range_type( std::begin( arguments), std::end( arguments)));
         }

         template< concepts::range R>
         requires std::convertible_to< std::ranges::range_value_t< R>, std::string_view>
         void operator() ( R arguments)
         {
            basic_parse::operator()( algorithm::transform( arguments, []( auto& value){ return std::string_view( value);}));
         }
