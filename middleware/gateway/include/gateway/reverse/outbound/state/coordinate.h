      ...
      namespace coordinate
      {
         namespace discovery
         {
            struct Message
            {
               struct Pending
               {
                  inline Pending( common::Uuid correlation, common::strong::file::descriptor::id source)
                     : correlation{ correlation}, source{ source} {}

                  common::Uuid correlation;
                  common::strong::file::descriptor::id source;

                  inline friend bool operator == ( const Pending& lhs, const common::Uuid& rhs) { return lhs.correlation == rhs;}
                  inline friend bool operator == ( const Pending& lhs, common::strong::file::descriptor::id rhs) { return lhs.source == rhs;}
               ...
               using callback_t = common::function< void( std::vector< common::message::gateway::domain::discover::Reply> received, std::vector< common::strong::file::descriptor::id> failed)>;
               ...
               inline bool failed( common::strong::file::descriptor::id connection)
               {
                  if( auto found = common::algorithm::find( m_pending, connection))
                  {
                     m_pending.erase( std::begin( found));
                     m_failed.push_back( connection);
                  }
                  return done();
               }
            ...
            private:
               ...
               std::vector< common::strong::file::descriptor::id> m_failed;
         ...
         struct Discovery
         {
            ...
            inline void failed( common::strong::file::descriptor::id connection)
            {
               common::algorithm::trim( m_messages, common::algorithm::remove_if( m_messages, [connection]( auto& message)
               {
