         ...
         namespace
         {
                  ...
                  struct Shared
                  {
                     ...
                     casual::task::unit::Dispatch try_finalize( Message&& message)
                     {
                        if( auto found = common::algorithm::find( correlations, message.correlation))
                        {
                           ...
                           else if constexpr( std::is_same_v< std::decay_t< Message>, ipc::message::forward::group::state::Reply>)
                              reply.forwards.push_back( std::forward< Message>( message));
                           else 
                              reply.fanouts.push_back( std::forward< Message>( message));
                        }
            ...
            void state( casual::manager::service::protocol::concurrent::Finalize< admin::model::State> finalize, manager::State& state)
            {
               ...
               auto action = casual::task::create::action( [ shared]( casual::task::unit::id)
               {
                  ...
                  std::ranges::transform( std::views::filter( shared->state->fanout.groups, is_running),
                     std::back_inserter( shared->correlations),
                     detail::state::send_request< ipc::message::fanout::group::state::Request>( *shared->state));
               ...
               auto handle_fanout_state = [ shared]( casual::task::unit::id, const ipc::message::fanout::group::state::Reply& message)
               {
                  Trace trace{ "queue::manager::admin::local::state::handle_fanout_state"};
