...
namespace casual
{
   ...
   namespace local
   {
      namespace
      {
         ...
         sf::service::protocol::parameter_type prepare( std::string protocol)
         {
            sf::service::protocol::parameter_type result;
         ...
         template< typename T>
         sf::service::protocol::parameter_type prepare( std::string protocol, T& value)
         {
            auto result = prepare( std::move( protocol));

            auto writer = sf::archive::writer::from::buffer( result.payload.memory, result.payload.type);
   ...
   TEST_P( protocol, deduce)
   {
      ...
      auto protocol = sf::service::protocol::deduce( local::prepare( GetParam()));
   ...
   TEST_P( protocol, simple_input_deserialize)
   {
      ...
      auto protocol = sf::service::protocol::deduce( local::prepare( GetParam(), some_long));
   ...
   TEST_P( protocol, input_deserialize)
   {
      ...
      auto protocol = sf::service::protocol::deduce( local::prepare( GetParam(), vo));
   ...
   TEST_P( protocol, output_serialize)
   {
...

      {
         ...
         auto reader = sf::archive::reader::from::buffer( result.payload.memory, result.payload.type);
